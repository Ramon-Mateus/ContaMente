...
export class ParcelaService {
  ...
  getParcelas(): Observable<Parcela[]> {
    return this.httpClient.get<Parcela[]>(this.urlDoModel, {
      withCredentials: true,
    })
  }

  getParcelaById(id: number): Observable<Parcela> {
    return this.httpClient.get<Parcela>(`${this.urlDoModel}/${id}`, {
      withCredentials: true,
    })
  }

  postParcela(parcela: postParcela): Observable<Parcela> {
    return this.httpClient.post<Parcela>(this.urlDoModel, parcela, {
      withCredentials: true,
    })
  }

  putParcela(id: number, parcela: postParcela): Observable<Parcela> {
    return this.httpClient.put<Parcela>(`${this.urlDoModel}/${id}`, parcela, {
      withCredentials: true,
    })
  }
