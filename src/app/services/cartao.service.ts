...
export class CartaoService {
  ...
  getCartoes(): Observable<Cartao[]> {
    return this.httpClient.get<Cartao[]>(this.urlDoModel, {
      withCredentials: true,
    })
  }

  getCartaoById(id: number): Observable<Cartao> {
    return this.httpClient.get<Cartao>(`${this.urlDoModel}/${id}`, {
      withCredentials: true,
    })
  }

  postCartao(cartao: PostPutCartao): Observable<Cartao> {
    return this.httpClient.post<Cartao>(this.urlDoModel, cartao, {
      withCredentials: true,
    })
  }

  putCartao(id: number, cartao: PostPutCartao): Observable<Cartao> {
    return this.httpClient.put<Cartao>(`${this.urlDoModel}/${id}`, cartao, {
      withCredentials: true,
    })
  }
