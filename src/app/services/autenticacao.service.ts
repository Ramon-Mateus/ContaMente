...
export class AutenticacaoService {
  ...
  login(usuario: Usuario_login, useCookies: boolean = true): Observable<any> {
    return this.httpClient.post<Usuario>(
      `${this.urlDoModel}/login?useCookies=${useCookies}`,
      usuario,
  ...
  logout(): Observable<void> {
    return this.httpClient
      .post<void>(`${this.urlDoModel}/Auth/logout`, null, {
        withCredentials: true,
      })
      .pipe(
        tap(() => {
  ...
  forgotPassword(email: string): Observable<void> {
    return this.httpClient.post<void>(
      `${this.urlDoModel}/Auth/forgotPassword`,
      { email }
    )
  }
  ...
  resetPassword(data: resetPasswordForm): Observable<void> {
    return this.httpClient.post<void>(
      `${this.urlDoModel}/Auth/resetPassword`,
      data
    )
  }
  ...
  verificarSessao(): Observable<boolean> {
    return this.httpClient
      .get<boolean>(`${this.urlDoModel}/Auth/verificarSessao`, {
        withCredentials: true,
      })
      .pipe(
        tap((autenticado) => {
  ...
  getUser(): Observable<Usuario> {
    return this.httpClient.get<Usuario>(`${this.urlDoModel}/Auth/getUser`, {
      withCredentials: true,
    })
  }
}
