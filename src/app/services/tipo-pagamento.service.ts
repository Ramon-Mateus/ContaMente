...
export class TipoPagamentoService {
  ...
  getTiposPagamento(): Observable<TipoPagamento[]> {
    return this.httpClient.get<TipoPagamento[]>(this.urlDoModel, { withCredentials: true });
  }

  getTipoPagamentoById(id: number): Observable<TipoPagamento> {
    return this.httpClient.get<TipoPagamento>(`${this.urlDoModel}/${id}`, { withCredentials: true });
  }
}
