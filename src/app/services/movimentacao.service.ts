...
export class MovimentacaoService {
  ...
  getMovimentacoes(
    ...
    responsaveisIds: number[] = [],
    cartoesIds: number[] = []
  ): Observable<{ movimentacoes: DiaFiscal[]; total: number }> {
    ...
    params = params.set('entrada', entrada === true ? 'true' : 'false')
    ...
    return this.httpClient
      .get<{ [Key: string]: Movimentacao[] }>(this.urlDoModel, {
        params,
        withCredentials: true,
      })
      .pipe(
        map((response: { [key: string]: Movimentacao[] }) => {
          const movimentacoes: DiaFiscal[] = Object.keys(response).map(
            (key) => {
              return {
                data: new Date(key),
  ...
  getMovimentacaoById(id: number): Observable<Movimentacao> {
    return this.httpClient.get<Movimentacao>(`${this.urlDoModel}/${id}`, {
      withCredentials: true,
    })
  }

  postMovimentacao(movimentacao: PostMovimentacao): Observable<Movimentacao> {
    return this.httpClient.post<Movimentacao>(this.urlDoModel, movimentacao, {
      withCredentials: true,
    })
  }
  ...
  putMovimentacao(
    ...
    return this.httpClient.put<Movimentacao>(
      `${this.urlDoModel}/${id}`,
      movimentacao,
      { withCredentials: true }
    )
