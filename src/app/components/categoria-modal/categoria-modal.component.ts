...
export class CategoriaModalComponent {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['visible'] && changes['visible'].currentValue === true && this.categoriaId > 0) {
      this.categoriaService.getCategoriaById(this.categoriaId).subscribe(categoria => {
        this.categoria = {
          nome: categoria.nome!,
  ...
  OnCreateCategoriaSubmit() {
    if(this.categoriaId === 0) {
      ...
      this.categoriaService.putCategoria(this.categoriaId, this.categoria).subscribe(() => {
        this.submit.emit();
        this.visible = false;
      });
    }
